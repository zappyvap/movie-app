...
interface Props { // props that are inputted from app.tsx
  query: string;
...
const SearchBar: React.FC<Props> = ({ query, setQuery, setMovies }) => {
  ...
  const onClick = () => {
    ...
    const options = { // this just uses an authorization key so I can fetch the data
      method: "GET",
      headers: {
    ...
    fetch(url, options)
      ...
      .then((json) => { // takes the results of the json and adds it to the movie list
